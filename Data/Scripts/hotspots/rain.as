...
void SetParameters() {
    params.AddFloatSlider("Lightning Time Multiplier",1.0f,"min:0.5,max:10,step:0.1");
    params.AddIntCheckbox("Muffled Sound", false);
}

void Update() {
    if(next_lightning_time < the_time){
        next_lightning_time = the_time + RangedRandomFloat(6.0, 12.0) * params.GetFloat("Lightning Time Multiplier");//RangedRandomFloat(3.0, 6.0);
    ...
    if(thunder_time < the_time && thunder_time != -1.0){
        if(lightning_distance < 0.3){
            if( params.GetInt("Muffled Sound") == 1 ){
                PlaySoundGroup("Data/Sounds/weather/muffled/thunder_strike_mike_koenig.xml", _sound_priority_high);
            } else {
                PlaySoundGroup("Data/Sounds/weather/thunder_strike_mike_koenig.xml", _sound_priority_high);
            }
        } else {
            if( params.GetInt("Muffled Sound") == 1 ){
                PlaySoundGroup("Data/Sounds/weather/muffled/thunder.xml", _sound_priority_high);
            } else {
