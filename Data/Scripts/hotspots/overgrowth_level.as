...
void SetParameters() {
    ...
    if(music_param_value == "slaver"){
        ...
        if(FileExists(g_music_custom_xml_filename)) {
            ...
            // Can't call GetSong() or GetLayerNames() to detect an error immediately since music load happens async.
            // Could continuously poll, but that would slow down gameplay (slightly, but more than none).
            g_music_settings_are_valid = true;
        } else {
        ...
        if(!g_music_settings_are_valid) {
            // Setting to silence, but not mucking with params so that modder can correct them
...
void TriggerGoalString(const string &in goal_str, GoalTriggerType type){
    ...
    while(token_iter.FindNextToken(goal_str)){
        if(token_iter.GetToken(goal_str) == "dialogue" && !EditorModeActive() && !Online_IsClient()){
...
const array<string> g_music_track_param_options = {
    ...
    "rescue",
    "arena",
    "custom_og_1_5"  // Make sure this is always the last parameter if you edit this list
};
const int kCustomMusicTrackParamIndex = int(g_music_track_param_options.length()) - 1;
const array<string> g_music_track_names = {
    // Make sure this always matches 1:1 with g_music_track_param_options if you edit this list
    "No Music",
    "Slaver",
    "Swamp",
    "Cats",
...
// Returns a 4 bit hex value as a string, e.g. 12 is "C"
// value param: 0 - 15
...
// Returns a 8 bit hex value as a string, e.g. 195 is "C3"
// value param: 0 - 255
string HexFromByte_(int value) {
    return HexFromDigit_(value / 16) + HexFromDigit_(value % 16);
}

// Returns a color escape code for use in Dear ImGui, e.g. vec4(1.0, 0.0, 1.0, 1.0) is "\x1BFF00FFFF" (opaque magenta)
// color param: 0.0 - 1.0 in each color channel, e.g. vec4(0.0, 1.0, 1.0, 1.0) (opaque cyan)
...
// Keeping these around so we can lazy-update them, instead of updating every frame
...
void DrawEditor() {
    ...
    if(g_is_editor_enabled) {
        ...
        if(g_music_params_are_dirty) {
            // Doing these every frame is slow, so lazy-updating them
        ...
        if(music_param_index == kCustomMusicTrackParamIndex) {
            ...
            if(ImGui_IsItemHovered()) {
                ImGui_SetTooltip(
                    ...
                    "\n  that contains a " + orange_color + "a multi-layer track" + plain_color + " of music." +
                    "\n" +
            ...
            // This isn't that slow, and it doesn't update immediately, so going ahead and polling it here in the UI
            ...
            if(ImGui_IsItemHovered()) {
                ImGui_SetTooltip(
                    ...
                    "\n" +
                    "\n  " + red_color + "<Song name=\"" + orange_color + "slavers1" + red_color + "\" type=\"layered\">" +
