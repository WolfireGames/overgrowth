...
void main() {   
    ...
    frag_tex_coordsB.x += sin(frag_tex_coordsB.y + time * 2) * 0.01;     //makes texture move back and fourth
    ...
    vec4 colormap = texture(tex0,frag_tex_coords);
        ...
        #else 
            vec4 normalmap = texture(tex1,frag_tex_coordsB);
    ...
    float spec_amount;
    //CalculateDecals(colormap, ws_normal, spec_amount, world_vert);
    ...
    diffuse_color += ambient_color * GetAmbientContrib(shadow_tex.g);
        ...
        #ifdef ALPHA
            spec_amount = normalmap.a;
        #else
            spec_amount = colormap.a;
        #endif
