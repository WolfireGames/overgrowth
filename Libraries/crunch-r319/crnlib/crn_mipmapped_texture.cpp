...
namespace crnlib
{
   ...
   bool mipmapped_texture::read_dds(data_stream_serializer& serializer)
   {
      ...
      {
         // clear destroys error message, save it
         dynamic_string saved_error;
         saved_error.swap(m_last_error);
         clear();
         saved_error.swap(m_last_error);
         return false;
      }
         ...
         switch (desc.ddpfPixelFormat.dwFourCC)
         {
            ...
            {  // DX10 header
               ...
               switch (dx10.dxgiFormat)
               {
                  ...
                  {
                     // FIXME: we don't have a way of communicating if we're sRGB
                  ...
                  default:
                  {
                     dynamic_string err_msg(cVarArg, "Unsupported DX10 format: 0x%u", dx10.dxgiFormat);
