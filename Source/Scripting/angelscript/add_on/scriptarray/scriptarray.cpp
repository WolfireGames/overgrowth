...
static void RegisterScriptArray_Native(asIScriptEngine *engine) {
    ...
    assert(r >= 0);
    r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in, uint length)", asFUNCTIONPR(CScriptArray::Create, (asITypeInfo *, asUINT), CScriptArray *), asCALL_CDECL);
    assert(r >= 0);
...
static void RegisterScriptArray_Generic(asIScriptEngine *engine) {
    ...
    assert(r >= 0);
    r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in, uint length)", asFUNCTION(ScriptArrayFactory2_Generic), asCALL_GENERIC);
    assert(r >= 0);
