...
DebugDraw::DebugDraw() : id_index(0) {}
...
DebugDrawLine::DebugDrawLine(
    ...
    const vec4 &start_color,
    const vec4 &end_color,
    const DDFlag &_flags) : DebugDrawWire((DDFlag)(_flags | _DD_COLOR)) {
...
DebugDrawLines::DebugDrawLines(
    ...
    const vec4 &_color,
    const DDFlag &_flags) : DebugDrawWire(_color, _flags) {
...
DebugDrawWireCylinder::DebugDrawWireCylinder(const vec3 &_position,
                                             const float _radius,
                                             const float _height,
                                             const vec4 &_color) : DebugDrawWire(_color, _DD_NO_FLAG) {
...
DebugDrawWireSphere::DebugDrawWireSphere(const mat4 _transform,
                                         const vec4 &_color) : DebugDrawWire(_transform, _color, _DD_NO_FLAG) {
    if (!wire_sphere_vbo->valid()) {
        std::vector<float> vert_array;
        GetWireSphereVertArray(1.0f, 12, 12, vert_array);
        wire_sphere_vbo->Fill(kVBOFloat | kVBOStatic, vert_array.size() * sizeof(vert_array[0]), &vert_array[0]);
...
DebugDrawText::DebugDrawText(const vec3 &_position,
                             ...
                             const DDFlag &_flags,
                             const vec4 &_color) : DebugDrawElement(_flags),
                                                   scale(_scale),
                                                   color(_color) {
    if (!vbo->valid()) {
        const float origo[] = {
...
DebugDrawElement::DebugDrawElement(const DDFlag &_flags) : flags(_flags),
                                                           fade_amount(0.0f),
                                                           type(kUndefined),
                                                           visible(true) {}
...
DebugDrawWire::DebugDrawWire(
    const DDFlag &_flags) : DebugDrawElement(_flags),
                            transform(1.0f),
...
DebugDrawWireBox::DebugDrawWireBox(const vec3 &pos,
                                   const vec3 &dim,
                                   const vec4 &color) : DebugDrawWire(color, _DD_NO_FLAG) {
...
DebugDrawWireMesh::DebugDrawWireMesh(const RC_VBOContainer &_vbo, const mat4 &_transform, const vec4 &_color) : DebugDrawWire(_vbo, _transform, _color, _DD_NO_FLAG) {
    type = kWireMesh;
}
...
DebugDrawCircle::DebugDrawCircle(
    const mat4 &_transform,
    const vec4 &_color,
    const DDFlag &_flags) : DebugDrawWire(_transform, _color, _flags) {
...
DebugDrawStippleMesh::DebugDrawStippleMesh(const RC_VBOContainer &_vbo,
                                           const mat4 &_transform,
                                           const vec4 &_color,
                                           const DDFlag &_flags) : DebugDrawElement(_DD_NO_FLAG),
                                                                   vbo(_vbo),
                                                                   transform(_transform),
                                                                   color(_color) {
    type = kStippleMesh;
