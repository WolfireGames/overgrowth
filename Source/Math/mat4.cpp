...
vec4 mat4::operator*(const vec4 &rhs) const {
    ...
    if (entries[3] == 0.0f && entries[7] == 0.0f && entries[11] == 0.0f && entries[15] == 1.0f) {
        return vec4(entries[0] * rhs.x() + entries[4] * rhs.y() + entries[8] * rhs.z() + entries[12] * rhs.w(),
...
mat4 mat4::GetInverseTranspose(void) const {
    ...
    result.entries[0] = tmp[0] * entries[5] + tmp[3] * entries[6] + tmp[4] * entries[7] - tmp[1] * entries[5] - tmp[2] * entries[6] - tmp[5] * entries[7];

    result.entries[1] = tmp[1] * entries[4] + tmp[6] * entries[6] + tmp[9] * entries[7] - tmp[0] * entries[4] - tmp[7] * entries[6] - tmp[8] * entries[7];

    result.entries[2] = tmp[2] * entries[4] + tmp[7] * entries[5] + tmp[10] * entries[7] - tmp[3] * entries[4] - tmp[6] * entries[5] - tmp[11] * entries[7];

    result.entries[3] = tmp[5] * entries[4] + tmp[8] * entries[5] + tmp[11] * entries[6] - tmp[4] * entries[4] - tmp[9] * entries[5] - tmp[10] * entries[6];

    result.entries[4] = tmp[1] * entries[1] + tmp[2] * entries[2] + tmp[5] * entries[3] - tmp[0] * entries[1] - tmp[3] * entries[2] - tmp[4] * entries[3];

    result.entries[5] = tmp[0] * entries[0] + tmp[7] * entries[2] + tmp[8] * entries[3] - tmp[1] * entries[0] - tmp[6] * entries[2] - tmp[9] * entries[3];

    result.entries[6] = tmp[3] * entries[0] + tmp[6] * entries[1] + tmp[11] * entries[3] - tmp[2] * entries[0] - tmp[7] * entries[1] - tmp[10] * entries[3];

    result.entries[7] = tmp[4] * entries[0] + tmp[9] * entries[1] + tmp[10] * entries[2] - tmp[5] * entries[0] - tmp[8] * entries[1] - tmp[11] * entries[2];
    ...
    result.entries[8] = tmp[0] * entries[13] + tmp[3] * entries[14] + tmp[4] * entries[15] - tmp[1] * entries[13] - tmp[2] * entries[14] - tmp[5] * entries[15];

    result.entries[9] = tmp[1] * entries[12] + tmp[6] * entries[14] + tmp[9] * entries[15] - tmp[0] * entries[12] - tmp[7] * entries[14] - tmp[8] * entries[15];

    result.entries[10] = tmp[2] * entries[12] + tmp[7] * entries[13] + tmp[10] * entries[15] - tmp[3] * entries[12] - tmp[6] * entries[13] - tmp[11] * entries[15];

    result.entries[11] = tmp[5] * entries[12] + tmp[8] * entries[13] + tmp[11] * entries[14] - tmp[4] * entries[12] - tmp[9] * entries[13] - tmp[10] * entries[14];

    result.entries[12] = tmp[2] * entries[10] + tmp[5] * entries[11] + tmp[1] * entries[9] - tmp[4] * entries[11] - tmp[0] * entries[9] - tmp[3] * entries[10];

    result.entries[13] = tmp[8] * entries[11] + tmp[0] * entries[8] + tmp[7] * entries[10] - tmp[6] * entries[10] - tmp[9] * entries[11] - tmp[1] * entries[8];

    result.entries[14] = tmp[6] * entries[9] + tmp[11] * entries[11] + tmp[3] * entries[8] - tmp[10] * entries[11] - tmp[2] * entries[8] - tmp[7] * entries[9];

    result.entries[15] = tmp[10] * entries[10] + tmp[4] * entries[8] + tmp[9] * entries[9] - tmp[8] * entries[9] - tmp[11] * entries[10] - tmp[5] * entries[8];

    // calculate determinant
    det = entries[0] * result.entries[0] + entries[1] * result.entries[1] + entries[2] * result.entries[2] + entries[3] * result.entries[3];
...
vec4 AxisAngleFromMat4(const mat4 &m) {
    ...
    if ((std::fabs(m(0, 1) - m(1, 0)) < epsilon) && (std::fabs(m(0, 2) - m(2, 0)) < epsilon) && (std::fabs(m(1, 2) - m(2, 1)) < epsilon)) {
        // singularity found
        // first check for identity matrix which must have +1 for all terms
        //  in leading diagonaland zero in other terms
        if ((std::fabs(m(0, 1) + m(1, 0)) < epsilon2) && (std::fabs(m(0, 2) + m(2, 0)) < epsilon2) && (std::fabs(m(1, 2) + m(2, 1)) < epsilon2) && (std::fabs(m(0, 0) + m(1, 1) + m(2, 2) - 3) < epsilon2)) {
    ...
    float s = sqrtf((m(2, 1) - m(1, 2)) * (m(2, 1) - m(1, 2)) + (m(0, 2) - m(2, 0)) * (m(0, 2) - m(2, 0)) + (m(1, 0) - m(0, 1)) * (m(1, 0) - m(0, 1)));  // used to normalise
